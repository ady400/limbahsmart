import time import numpy as np import pandas as pd import streamlit as st import io from streamlit_lottie import st_lottie import requests
...
lottie_beranda = load_lottie_url("https://assets7.lottiefiles.com/packages/lf20_2glqweqs.json") lottie_lab = load_lottie_url("https://assets1.lottiefiles.com/packages/lf20_kkflmtur.json") lottie_simulasi = load_lottie_url("https://assets4.lottiefiles.com/packages/lf20_iwmd6pyr.json")
...
st.markdown(""" <style> .main-title { font-size: 36px; color: #2C3E50; text-align: center; padding: 20px 0; animation: fadeIn 2s; } .stButton>button { background-color: #2C3E50; color: white; } body { background-color: #f5f9ff; } @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } } </style> """, unsafe_allow_html=True)

BERANDA

if menu == "🏠 Beranda": st_lottie(lottie_beranda, speed=1, loop=True, quality="high", height=250) st.markdown(""" <div style='text-align: center; padding: 30px 0;'> <h1 style='color:#2C3E50;'>♻ Aplikasi Pengolahan Limbah Industri ♻</h1> <p style='font-size:18px; color:#555;'>Belajar dan simulasi proses pengolahan limbah industri secara interaktif dan edukatif.</p> </div> """, unsafe_allow_html=True)
...
elif menu == "⚙ Proses": st.markdown('<div class="main-title">⚙ Tahapan Pengolahan Limbah Industri</div>', unsafe_allow_html=True) st.markdown(""" ### 🧹 1. Pra-Pengolahan (Pre-Treatment) - Screening: Menyaring benda kasar seperti plastik dan kayu. - Grit Chamber: Mengendapkan partikel berat seperti pasir. - Equalization Tank: Menyeimbangkan aliran dan beban limbah.
...
elif menu == "🧪 Uji Lab": st_lottie(lottie_lab, speed=1, loop=True, quality="high", height=200) st.markdown('<div class="main-title">🧪 Kalkulator Uji Laboratorium</div>', unsafe_allow_html=True) uji = st.selectbox("Pilih jenis uji:", ["COD", "BOD", "TSS", "pH"])
...
elif menu == "🧩 Simulasi": st_lottie(lottie_simulasi, speed=1, loop=True, quality="high", height=200) st.markdown('<div class="main-title">🔄 Simulasi Pengolahan Limbah</div>', unsafe_allow_html=True) jenis = st.selectbox("Jenis limbah", ["Organik", "Kimia", "Campuran"]) awal = st.number_input("Konsentrasi awal (mg/L)", value=500.0)
